...
export function MonthlyReport({ income, expenses, emergencyFund }: MonthlyReportProps) {
  ...
  const totalExpenses = validExpenses.reduce((sum, expense) => {
    // Handle both 'value' and 'amount' properties for backwards compatibility
    const expenseAmount = safeNumber(expense.value) || safeNumber(expense.amount);
  ...
  if (validExpenses.length > 0) {
    const expenseWithAmounts = validExpenses.map(expense => ({
      ...
      amount: safeNumber(expense.value) || safeNumber(expense.amount)
    }));
  ...
  const reportData: FinancialReport = {
    ...
    categories: validExpenses.map(expense => ({
      name: expense.name || "مصروف غير محدد",
      amount: safeNumber(expense.value) || safeNumber(expense.amount),
      percentage: safeIncome > 0 ? ((safeNumber(expense.value) || safeNumber(expense.amount)) / safeIncome) * 100 : 0
    })),
    highestExpense: {
              ...
              <div className="mt-3">
                ...
                <div className="max-h-36 overflow-y-auto">
                  {validExpenses.map((expense, index) => {
                    const amount = safeNumber(expense.value) || safeNumber(expense.amount);
                    return (
