...
const [goalToDelete, setGoalToDelete] = useState<string | null>(null);
  ...
  <DialogContent className="sm:max-w-md">
    <DialogHeader>
      <DialogTitle className="text-right font-cairo">تأكيد حذف الهدف</DialogTitle>
    </DialogHeader>
    <div className="space-y-4 text-right font-cairo">
      <p>هل أنت متأكد من أنك تريد حذف هذا الهدف؟ لا يمكن التراجع عن هذا الإجراء.</p>
      <div className="flex gap-2 justify-end">
        <Button
          onClick={() => {
            if (goalToDelete) {
        ...
        >
          {deleteGoalMutation.isPending ? "جارٍ الحذف..." : "نعم، حذف"}
        </Button>
        <Button variant="outline" onClick={() => setGoalToDelete(null)}>إلغاء</Button>
      </div>
    </div>
  </DialogContent>
</Dialog>
