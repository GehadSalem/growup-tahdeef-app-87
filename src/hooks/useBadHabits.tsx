...
} from "@/services/badHabitsService";
...
export function useBadHabits() {
  ...
  const { data: badHabits = [], isLoading } = useQuery({
    queryKey: ["bad-habits"],
    queryFn: async () => {
      const response = await BadHabitsService.getBadHabits();
      return response.data; // Make sure to return the array, not the object
    },
  });
  // Create bad habit mutation
  const addBadHabitMutation = useMutation({
    mutationFn: (habitData: CreateBadHabitRequest) =>
      BadHabitsService.createBadHabit(habitData),
    onSuccess: (data) => {
  ...
  const updateBadHabitMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: Partial<CreateBadHabitRequest>;
  ...
  return {
    badHabits,
    isLoading,
    addBadHabit: (habitData: CreateBadHabitRequest) =>
      addBadHabitMutation.mutate(habitData),
